...
// 多租户相关：若传入 tenantCode，将以 X-Tenant-Code 头部发送；如不需要请保持 undefined
const login = (req: T.AccountLoginReq | T.PhoneLoginReq | T.EmailLoginReq, _tenantCode?: string) => {
  const headers: Record<string, string> = {}
  // 已按需求保留但注释说明：
  // if (tenantCode) {
  //   headers['X-Tenant-Code'] = tenantCode
  // }
  return http.post<T.LoginResp>(`${BASE_URL}/login`, req, { headers })
}
